...
- `Docs/Requisitos/` — software requirements for the character-sheet/campaign-management web app, all fleshed out:
  ...
  - `Requisitos - Ficha de Personagem.md` — the anchor document: specifies the character sheet field-by-field across 7 tabs (Informações Básicas, Atributos & Perícias, Combate, Magias & Habilidades, Posses, História, Diário). `Requisitos - Ficha de NPCs.md` and `Requisitos - Ficha de Criaturas.md` both define themselves as diffs against this doc rather than restating it — read it first.
