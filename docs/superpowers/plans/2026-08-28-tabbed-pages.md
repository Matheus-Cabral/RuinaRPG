...
- Button classification (apply uniformly, do not improvise per page):
  ...
  |---|---|---|
  | Primary form submit / create / attach / search | Salvar / Adicionar * / Criar * — **only when the button submits an `<EditForm>`** (e.g. Adicionar Item, Adicionar Magia/Habilidade, Adicionar Runa, Adicionar Maestria, Adicionar Afeição, Adicionar Característica, Adicionar Espólio, Adicionar Arma); Conceder, Anexar *, Buscar, Novo Encontro | `btn btn-primary` |
  ...
  Use `btn-sm` for buttons inside a `<li>`/table row; full-size `btn` for the button that submits an `<EditForm>`. **The label alone never decides the class — context does**: the same verb ("Salvar", "Adicionar", "Criar") can appear both as the one `<EditForm>`-submit action in a tab (→ `btn btn-primary`, full size) and as an inline `<li>`-row action (→ `btn btn-outline-primary btn-sm`). Two confirmed examples already in the codebase: (1) "Salvar" — the `<EditForm>`-submit Salvar is primary; the inline row-edit-confirm Salvar (paired with an inline Cancelar) is outline-sm — see `CampanhaDetalhe.razor`'s Diário/Notas Secretas tabs. (2) "Adicionar"/"Criar" — `CampanhaDetalhe.razor`'s Membros tab has "Adicionar" (attaching a searched player to the campaign) and "Criar Ficha de Personagem", both inline `<li>`-row actions, both `btn btn-outline-primary btn-sm` — despite the verb, they're not the tab's form-submit action. When in doubt: is this the one button that submits the tab's `<EditForm>`? Primary. Does it live inside a `<li>`/table row alongside other row actions? Outline + sm.
