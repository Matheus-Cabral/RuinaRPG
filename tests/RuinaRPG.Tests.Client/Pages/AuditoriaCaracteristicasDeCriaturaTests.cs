...
public class AuditoriaCaracteristicasDeCriaturaTests : MudBunitContext
{
    ...
    private IRenderedComponent<ContainerFragment> RenderWithDialogProvider()
    {
        var http = FakeHttpMessageHandler.CreateClient(request =>
            request.Method == HttpMethod.Get && request.RequestUri!.AbsolutePath.EndsWith("creature-exclusive-traits")
        ...
        return Render(builder =>
        {
            ...
            builder.OpenComponent<AuditoriaCaracteristicasDeCriatura>(1);
            builder.CloseComponent();
        });
    }
