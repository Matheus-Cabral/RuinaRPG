        ...
        IRulesDataProvider rules)
    {
        ...
        if (categorias is null || categorias.Contains(CompendioCategoria.Tabela))
        {
            ...
            results.AddRange(rules.CirculoGrauPorEap.Select(c =>
                new CompendioSearchResult(CompendioCategoria.Tabela, $"Tabela de Círculo e Grau por VIS — {c.CirculoOuGrau}", $"Círculo/Grau {c.CirculoOuGrau}",
                    $"VIS absoluto {c.EapAbsoluto}, VIS relativo {c.EapRelativo}, Afinidade {c.AfinidadeAbsoluta} (+{c.AfinidadeGanhoPorNivel}/nível)")));
            ...
            results.AddRange(rules.EapPorNivel.Select(e =>
                new CompendioSearchResult(CompendioCategoria.Tabela, $"Tabela de VIS por Nível — Nível {e.Nivel}", $"Nível {e.Nivel}", $"VIS {e.ValorAbsoluto}")));
        }
