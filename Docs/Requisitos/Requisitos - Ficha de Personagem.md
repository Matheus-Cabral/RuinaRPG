...
- Se a opção escolhida (Gratuita ou Obrigatória) exigir Especificação (mesma regra do campo livre acima) e "[[Requisitos - Habilidades Raciais]]" não tiver um valor já fixado para ela (como o "Fogo" da Desvantagem Elemental do Alóra), o popup mostra um campo de texto livre para esse valor; confirmar sem preenchê-lo é rejeitado (mesmo comportamento do campo livre acima — não há bloqueio no próprio botão).
